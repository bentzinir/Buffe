...
class ENVIRONMENT(object):
    ...
    def step_loss(self, state, action, time_):
        # cost:
        ...
        cost_acci_d = tf.mul(tf.pow(self.gamma, time_), cost_acci)
    ...
    def render(self):
        ...
        r = self.r
        v_h = state[self.v_h_field[0]]
        x_h = state[self.x_h_field[0]]
        v_t = state[self.v_t_field]
        x_t = state[self.x_t_field]
        is_aggressive = state[self.is_aggressive_field]

        x_h_w = (x_h > 0) * r * np.array([np.cos(x_h / r - np.pi / 2), np.sin(x_h / r - np.pi / 2)]) + \
                       (x_h <= 0) * np.array([0, -r + x_h])

        x_t_w = (r * np.cos(x_t / r - np.pi / 2), r * np.sin(x_t / r - np.pi / 2))

        self.ax.scatter(x_h[0], x_h[1], s=180, marker='o', color='r')
        self.ax.annotate(v_h, (x_h[0], x_h[1]))

        for tx, ty, v_, is_ag in zip(x_t_w[0], x_t_w[1], v_t, is_aggressive):
            if is_ag:
    ...
    def play_trajectory(self):
        ...
        for i, (x_h, v_h, x_t, v_t, c) in enumerate(zip(x_h_world, v_h_world, x_t_world, v_t_world, costs)):
            state = (x_h, v_h, x_t, v_t, is_aggressive)
