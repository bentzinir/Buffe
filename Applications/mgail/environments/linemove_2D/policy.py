...
class POLICY(object):
    ...
    def backward(self, loss):
        ...
        if self.solver_params['weight_decay']:
            loss += self.solver_params['weight_decay'] * tf.add_n([tf.nn.l2_loss(v) for v in self.trainable_variables.values()])

        # compute the gradients for a list of variables
        grads_and_vars = opt.compute_gradients(loss=loss, var_list=self.trainable_variables.values())
    ...
    def _init_layers(self):
        ...
        self.trainable_variables = dict(self.weights)
        self.trainable_variables.update(self.biases)

        weights, biases = self.create_variables()
        self.accum_grads = dict(weights)
    ...
    def copy_weights(self, values):
        self.copy_weights_op = []
        for key, value in self.trainable_variables.iteritems():
            self.copy_weights_op.append(value.assign(values[key]))
